...
import { useState, useEffect, useRef } from 'react' // Corrected import syntax
import Link from 'next/link'
import { Menu, X, ArrowRight, UserCircle, LogOut, Loader2 } from 'lucide-react' // Added UserCircle, LogOut, Loader2 icons
import { useRouter } from 'next/navigation' // Import useRouter for client-side navigation

const Navbar = () => {
  const [isOpen, setIsOpen] = useState(false) // State for mobile menu
  const [user, setUser] = useState(null) // State for authenticated user data
  const [loadingUser, setLoadingUser] = useState(true) // State for loading user profile
  ...
  return (
    <nav className="fixed top-0 w-full z-50 bg-[#0B132B]/80 backdrop-blur-lg border-b border-gray-800 font-['Inter',_sans-serif]">
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:block">
            <div className="ml-10 flex items-baseline space-x-8">
              {/* Courses Link as a distinct button-like element */}
              <Link
                href="/best-courses"
            ...
            {loadingUser ? (
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                >
                  ...
                  {/* Display full name, now visible on all desktop sizes */}
                ...
                {isDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-[#1A202C] rounded-lg shadow-xl py-2 border border-gray-700">
                    <Link
                      href="/dashboard" // Example: Link to user dashboard
        ...
        {isOpen && (
          <div className="md:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-[#0B132B]/95 backdrop-blur-lg rounded-lg mt-2">
              ...
              {/* Common Mobile Links (always visible regardless of auth status, adjust if needed) */}
              <Link
                href="/best-courses"
